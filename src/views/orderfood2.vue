...
<script>
    export default {
        methods: {
            ...
            page(currentPage){
                const _this=this
                axios.get('http://localhost:8030/client/Menuclient/findByTid/'+currentPage+'/5/2')
                    .then(function (resp){
                        console.log(resp);
        ...
        created() {
            const _this=this
            axios.get('http://localhost:8030/client/Menuclient/findByTid/1/5/2')
                .then(function (resp){
                    console.log(resp);
                    //  后端使用Page<>类型返回的，所有从响应对象的.data.content对象中取得数据
                    _this.tableData=resp.data;
                })
            axios.get('http://localhost:8030/client/Menuclient/countByTid/2')
                .then(function (resp){
                    console.log(resp);
